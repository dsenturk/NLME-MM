###########################################################################################
## Description: Functions for fitting single-level NLME model via MM algorithm described
##              in 'Modeling intra-individual inter-trial EEG response variability in Autism 
##              Spectrum Disorder'.
###########################################################################################
## Functions included:
## Main function:
##    1. fit_nlme_mm!: Function that fits single-level NLME model via MM algorithm
## Supporting functions used by main function:
##    2. update_mm!: Function that performs one iteration of MM algorithm
##    3. trial_qr_parallel: Function that performs QR decomposition in Step1 of each iteration
##    4. mm_value_update_parallel: Function that calculates components for updating variance 
##                                 components in Step 2 of each iteration
##    5. start_nlme!: Function that calculates starting values of model components
###########################################################################################




function fit_nlme_mm!(
    ...
    halving_maxiter::Int,           # maximum number of iterations for step-halving procedure in Step 1
    ...
    ##              sigma2: estimated variance of measurement error (scalar)
    #########################################################################################
    ...
    # Perform 1st iteration and save approximated marginal log-likelihood into obj
    m.iter = 1
    obj = update_mm!(m, gn_maxiter, gn_err_threshold, halving_maxiter)
    println("iter=1", "obj=", obj)  # print current evaluation of approximated marginal log-likelihood
...
function update_mm!(
    ...
    ##              in NlmeModel type object input, and returns approximated marginal log-
    ...
    while m.gn_iter <= gn_maxiter && m.gn_err >= gn_err_threshold
        ...
        @sync @distributed for r = 1:m.R
            ...
            m.s_1[(5*r-4):(5*r)],
            m.S_0[(T*r-(T-1)):(T*r),:],
            m.s_0[(T*r-(T-1)):(T*r)] = trial_qr_parallel(m.beta_w, m.gamma_w[:,r], m.Delta, m.y_mt[:,r], m.t, m.phi_r,
    ...
    @sync @distributed for r in 1:m.R
        m.mm_update_value[:,r] .= mm_value_update_parallel(m.beta, m.gamma[:,r], m.y_mt[:,r], m.t, 
...
function trial_qr_parallel(
    ...
    # Derive design matrices of r-th trial-level random effects
    K_r .= [M_r; Delta]
...
function mm_value_update_parallel(
    # current estimation of model components
...
function start_nlme!(
    ...
    #   find_erp: Function that detects peak- or dip-shaped components from a single curve
    #             based on given searching interval and returns its latency and amplitude
    ...
    m.beta .= vec(mapslices(mean, m.gamma, dims = 2))
    m.gamma .-= m.beta
    m.V .= Diagonal([vec(mapslices(var, m.gamma[1:4,:], dims = 2));10]) 
                    # constant 10 is set to avoid potential explosion in trial-level variance of mean magnitude
    m.gamma = zeros(5, m.R)
    m.sigma2 = 2    
    m.V_half .= (m.V).^(0.5)
    m.V_inv .= inv(m.V)
