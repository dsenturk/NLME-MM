...
## Functions included:
## Main function:
##    1. fit_mnlme_mm!: Function that fits multi-level NLME model via MM algorithm
## Supporting functions used by main function:
##    2. update_mnlme_mm!: Function that performs MM iteration
##    3. subject_qr_parallel: Function that performs QR decomposition in Step1 of each iteration
##    4. mnlme_mm_update_parallel: Function that calculates components for updating variance 
##                                 components in Step 2 of each iteration
##    5. start_mnlme!: Function that calculates starting values of model components from
##                     single-level model fits
###########################################################################################



function fit_mnlme_mm!(
    m::MnlmeModel{FloatType},       # MnlmeModel type object constructed by MnlmeModel_construct
                                    ...
                                    #       gamma: current estimates of trial-level random effects for each subject 
    ...
    # 'update_mnlme_mm!': Function that performs MM iteration for multi-level NLME, overwrites
...
function update_mnlme_mm!(
    ...
    # Assign each subset to a worker
    ...
    while m.gn_iter <= gn_maxiter && m.gn_err >= gn_err_threshold
        ...
        @inbounds for i = 1:m.n
            ...
            futures[i] = remotecall(subject_qr_parallel, wks_schedule[i],
    ...
    # End of Step 1: Overwrite fixed and random effects in MnlmeModel object with estimations
    ...
    @inbounds for i=1:m.n
        futures[i] = remotecall(mnlme_mm_update_parallel, wks_schedule[i],
                                vec(m.beta), 
...
function mnlme_mm_update_parallel(
    ...
    @inbounds for r=1:R_i
        ...
        # Trial-specific contribution to approximated marginal log-likelihood (derivation see Supplementary A)
    ...
    # Calculate inverse of approximated subject-level covariance matrix via Woodbury matrix identity
...
function start_mnlme!(
    ...
    for subject_index = 1:m.n
        # Construct `NlmeModel` type variable (see definition in "NLME_classfile.jl")
